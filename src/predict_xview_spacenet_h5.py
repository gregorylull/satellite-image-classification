...
split_image_folder = 'snippet_test_images_split128_bit255_float16'
...
CACHED_PREDICTION_FILENAME = os.path.join(
    'models_xview2', f'xview2{parameters}_preds.pkl')
...
def plot_sample(images, preds, binary_preds, ix=None):
    ...
    pre_start = pre_index * square_dim
    pre_end = pre_start + square_dim

    post_start = post_index * square_dim
    post_end = post_start + square_dim

    # original images
    pre_image = gldata.stich_images(
        images[pre_start:pre_end]).astype(np.int)
    post_image = gldata.stich_images(
        images[post_start:post_end]).astype(np.int)

    # predicted images
    pre_binary_preds_image = gldata.stich_images(
        binary_preds[pre_start:pre_end]).astype(np.float32)
    ...
    ax[1, 0].imshow(pre_binary_preds_image.squeeze(), vmin=0, vmax=1)
    ax[1, 1].imshow(post_binary_preds_image.squeeze(), vmin=0, vmax=1)
...
def output_train_val_test_images(
    ...
    # Check tests
    for i in range(1, min(CHECK_IMAGES, int(images.shape[0] / (split_len ** 2))), 2):
        fig_val = plot_sample(images, preds_test, preds_test_t, ix=i)
        name = f'{dirpath}/xview2_spacenet_test_predicted_fig_{i}_{parameters}.png'
